...
func TestRebuildItems_AllWorkspacesShowsAll(t *testing.T) {
	m := Model{
		folded: make(map[string]bool),
		filter: allOnFilter(),
		sessions: []proto.SessionInfo{
	...
	if sessionCount != 2 {
		t.Errorf("visible sessions = %d, want 2 (All)", sessionCount)
	}
}
...
func TestRebuildItems_ResetsSelectedWorkspaceWhenMissing(t *testing.T) {
	...
	if m.selectedWorkspace != "" {
		t.Errorf("selectedWorkspace = %q after workspace disappeared, want ''", m.selectedWorkspace)
	}
}
