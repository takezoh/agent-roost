...
func TestParseRoostWindows(t *testing.T) {
	...
	// tags, agent_session, agent_workdir, agent_transcript
	out := "@1\tabc123\t/tmp/proj\tclaude\t2026-04-08T12:00:00Z\t[]\t\t\t\n" +
		"@2\t\t\t\t\t\t\t\t\n" + // skipped: empty @roost_id
		"@3\tdef456\t/tmp/other\tclaude --worktree\t2026-04-08T12:01:00Z\t[{\"text\":\"main\"}]\tagent-789\t/tmp/other/.claude/worktrees/foo\t/home/u/.claude/projects/-tmp-other--claude-worktrees-foo/agent-789.jsonl\n"
...
func TestParseRoostWindows_TooFewFields(t *testing.T) {
	...
	// Pre-AgentWorkingDir 8-field format should now be skipped.
	if got := parseRoostWindows("@1\tabc\t/tmp/proj\tclaude\t2026-04-08T12:00:00Z\t[]\t\t\n"); len(got) != 0 {
		t.Fatalf("expected 0 windows for legacy 8-field line, got %d", len(got))
	}
}
