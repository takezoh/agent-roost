...
- `session.Manager` → `session/driver`（`Manager.Recreate` が cold-boot 復元時に `driver.Registry.Get(...).SpawnCommand(...)` を呼ぶ）
...
`Driver.ResolveMeta` は agent が報告した transcript file の絶対パスを直接読み取る。本番では `os.DirFS("/")` を渡し、テストでは `fstest.MapFS` を使う。`Driver.TranscriptFilePath` は agent がまだ hook event で transcript path を報告していない時 (cold-boot 直後など) の **fallback** 用で、driver が cwd と agentSessionID から自分の transcript file 配置ルールを使ってパスを組み立てる。Claude は `~/.claude/projects/{projectDir(cwd)}/{agentSessionID}.jsonl` を返し、Generic は `""` を返す。
...
type AgentEvent struct {
    ...
    WorkingDir     string  // agent process の実 cwd
    TranscriptPath string  // agent が書いている transcript file の絶対パス
...
各 driver subcommand (`roost claude event`, 将来の `roost gemini event` 等) が自分の hook payload (Claude なら JSON の `cwd` / `transcript_path` / ...) を `AgentEvent` 構造体に **詰め替えて** IPC で送信する。`core.Server.handleAgentEvent` は冒頭で `driver.AgentEventFromArgs(args)` を呼ぶだけで、それ以降は struct フィールド経由で値にアクセスする。**core から見て driver 固有の field 名 (Claude の `cwd` 等) は一切現れない**。これが driver 抽象化の唯一の境界。
...
`Session.AgentWorkingDir` と `Session.AgentTranscriptPath` は agent ランタイム情報で、`AgentEvent` 経由で driver から届いた値を `Manager.SetAgentWorkingDir` / `Manager.SetAgentTranscriptPath` が tmux user options (`@roost_agent_workdir`, `@roost_agent_transcript`) に永続化する。`SetAgentWorkingDir` は branch tag の再計算もトリガー (`refreshSessionBranchLocked` が `AgentWorkingDir` を優先するため、`claude --worktree` で起動した瞬間に worktree branch に切り替わる)。`Service.transcriptPathFor` は `AgentTranscriptPath` (driver が報告した真実) を最優先し、空の場合のみ `Driver.TranscriptFilePath` で `AgentWorkingDir` (なければ `Project`) から fallback 計算する。
...
| driver hook payload の抽象化 | `driver.AgentEvent` 構造体に詰め替え | 各 driver subcommand が tool 固有の hook field (Claude の `cwd` / `transcript_path` 等) を `AgentEvent.WorkingDir` / `AgentEvent.TranscriptPath` 等の中立フィールドへ翻訳。core/server.go は `AgentEventFromArgs` で構造体を取り出すだけで、driver 固有の string key を直読しない。これが Claude/将来の Gemini 等の依存吸収境界 |
| Session ランタイム情報の保持 | `Session.AgentWorkingDir` と `Session.AgentTranscriptPath` を tmux user option (`@roost_agent_workdir` / `@roost_agent_transcript`) と sessions.json で永続化 | git branch 検出は `AgentWorkingDir` 起点 (なければ `Project` フォールバック)。transcript path は `AgentTranscriptPath` を最優先で使い、未取得時のみ `Driver.TranscriptFilePath` で計算する |
...
| `~/.config/roost/sessions.json` | JSON | セッション一覧の cold-boot スナップショット | Manager の各ミューテーション (Create/Stop/SetAgentSessionID/SetAgentWorkingDir/SetAgentTranscriptPath/RefreshBranch/Refresh) で書き出し。読まれるのは PC 再起動 (`!client.SessionExists()`) 時の `Manager.Recreate` のみ。runtime の真実は tmux user options |
...
├── session/
│   ├── manager.go       セッション CRUD。runtime は tmux user options 経由 (Refresh / SyncBranches / SetAgentSessionID)、cold-boot は sessions.json から Recreate
│   ├── state.go         状態 enum + Session struct（tmux 固有データ + AgentSessionID）+ RoostWindow 型
│   ├── log.go           ログパスヘルパー
│   └── driver/
│       ├── driver.go    Driver インターフェース (Name, PromptPattern, DisplayName, ResolveMeta, SpawnCommand)
...
│       ├── claude.go    Claude driver (❯/> のみ、$ 除外。JSONL からタイトル・プロンプト・TaskCreate subject 解析)
