...
- **プラガブル登録パターン**: driver のプラグイン登録は `RegisterTabRenderer[C]`, `RegisterRunner[In,Out]`, `state.Register(Driver)` の 3 つの init 時レジストリで行う。runtime/tui は具体的な driver を知らずにレジストリ経由で dispatch する
...
- `runtime/worker` → `state` (JobID, JobInput, EvJobResult)。driver/lib を import しない
...
- `tui` → `proto` (Client + SessionInfo), `state` (Status/View/TabRenderer 型), `tools` (ToolRegistry)。driver/lib を import しない
